...
public class FinalAns extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		//ValidateUser.rootPath = request.getServletContext().getContextPath();
		...
		if (samlRequest == null) {
			...
			if (status.equals("valid")) {
				...
				System.out.println("user is valid and mailId.....in session");
				HttpSession session = request.getSession(true);
				session.setAttribute("username", mailId);
				// String genSamlResponse=SamlResponse.genSamlResponse(mailId);
				response.sendRedirect("ssoredirect.jsp");
				return;
			}
		} else {
			...
			try {
				...
				samlResponse = getResponse(acsUrl, requestId,sessionIndex);
	...
	private Response getResponse(String acsUrl, String requestId,String sessionIndex) throws IOException, ConfigurationException, MarshallingException, XPathExpressionException, GeneralSecurityException, XMLSecurityException {
		Status status = SAMLBuilder.buildStatus(StatusCode.SUCCESS_URI);

		// String entityId = "http://localhost:8080/SAML";
		String entityId = "http://localhost:8080/SAML";
		// Response authResponse = SAMLBuilder.buildSAMLObject(Response.class,
		// Response.DEFAULT_ELEMENT_NAME);
		//Response authResponse = new ResponseBuilder().buildObject(SAMLConstants.SAML20_NS,SAMLConstants.SAML20P_PREFIX);
	...
	//	authResponse("urn:oasis:names:tc:SAML:2.0:assertion");
		authResponse.setID(SAMLBuilder.randomSAMLId());
		// authResponse.setID("0oa8afzkun589XwAm5d6");
		...
		Assertion assertion = buildAssertion(sessionIndex);
		authResponse.getAssertions().add(assertion);
		// authResponse.getAssertions().add(e)asserrt // InputStreamReader in_strm = new
		// InputStreamReader((InputStream) assertion);
		System.out.println("Assertiob is \t" + assertion);
		
		authResponse.setDestination(acsUrl);

		authResponse.setStatus(status);
		// authResponse.setAssertion(assertion);
	...
	public Assertion buildAssertion(String sessionIndex) throws ConfigurationException {
		...
		// nameId.setValue("okta123");
		// nameId.setNameQualifier("http://localhost:8080/SAML/SSO");
		...
		nameId.setValue("jaiiselvaraj@gmail.com");
		nameId.setFormat(NameID.UNSPECIFIED);
	...
	//	authStmt.;;
		// authStmt.setSessionNotOnOrAfter(now2.plus(input.getMaxSessionTimeoutInMinutes()));
		...
		//Condition condition=new ConditionBuilder().buildObject();
       ...
       if(attributes != null){
           ...
           while (keySet.hasNext() ){
              ...
              // String s=AttributeValue.DEFAULT_ELEMENT_LOCAL_NAME ;
              // attrFirstName.setName(s);
	...
	private Map<String, String> processSamlRequest(String samlRequest) throws Exception {
		...
		// String objJson = new ObjectMapper().writeValueAsString(obj);
	...
	/*private void sign(Assertion assertion) {
		...
		try {
			credential = getCredential("http://localhost:8080/SAML", "keystorepass", "saml-private-key.key");
		} catch (Exception e) {
			e.printStackTrace();
		}
		try {
			signAssertion(assertion, credential);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}*/
...
public Signature signResponse(Response response)  {
   ...
   // String keyInfoGeneratorProfile = "XMLSignature";
	...
	/*private Credential getCredential(String entityId, String password, String keyStorePath) throws Exception {

		InputStream inputStream = getClass().getClassLoader().getResourceAsStream(keyStorePath);

		KeyStore keystore;
		System.out.println("Using keystorePath: " + keyStorePath);
